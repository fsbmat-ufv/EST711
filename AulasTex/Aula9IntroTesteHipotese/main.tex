...
\begin{align*}
    1-P(\text{erro tipo II})=\underbrace{P_{\theta_{1}}(d(\vec{X})=1)}_{\text{Poder do Teste para}~\theta}
\end{align*}
\end{block}
\end{frame}
...
\begin{frame}{}
	\begin{block}{}
		\justifying
		Uma vez definidos os conceitos de função poder, tamanho do teste e região crítica, nosso próximo objetivo é construir um teste de hipóteses que satisfaça um nível de significância pré-estabelecido. Assim, o desafio passa a ser encontrar uma região crítica adequada, de modo que o teste resultante apresente tamanho $\alpha$ e o maior poder possível contra as alternativas de interesse.
...
\begin{frame}{Determinação do valor crítico $c$}
	\justifying
	Suponhamos que $n = 16$ e que estamos interessados em fixar $\alpha = 0{,}05$. Então, para determinar $c$, temos que resolver a equação 
	\[
	\alpha = P_{H_0}(\Bar{X} \geq c),
	\]
	ou seja,
	\[
	0{,}05 = P_{H_0}(\Bar{X} \geq c) = P\left(Z \geq \frac{c - \mu_0}{\sigma/\sqrt{n}}\right),\Rightarrow 
	\frac{c - \mu_0}{\sigma/\sqrt{n}} = 1{,}64,
	\]
	pois, na distribuição $N(0,1)$, o valor $1{,}64$ é o percentil $95\%$. 
	
	Logo, se $\mu_0 = 0$ e $\sigma = 1$, então $c = 0{,}41$, de modo que
	\[
	A_1 = \{x; \Bar{x} \geq 0{,}41\}.
	\]
\end{frame}

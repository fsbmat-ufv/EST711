...
\begin{frame}{Exemplo 1: Distribuição Exponencial}
	...
	\begin{block}{}
		\justifying
		Seja \(X_1, \ldots, X_n\) uma amostra aleatória de \(X \sim \text{Exp}(\theta)\), com densidade
...
\begin{frame}{Exemplo 1: Distribuição Exponencial}
	\begin{block}{}
		...
		Para \(\theta_1 > \theta_0\), a razão acima é uma função \textbf{monótona não decrescente} em \(t(\vec{x}) = \sum x_i\). Portanto, a distribuição \(\text{Exp}(\theta)\) possui \textbf{RVM crescente} em \(t(\vec{x}) = \sum x_i\). Alternativamente, também possui \textbf{RVM não crescente} em \(t^*(\vec{x}) = -\sum x_i\).
	\end{block}
\end{frame}
...
\begin{frame}{Teorema de Karlin-Rubin}
	...
	\begin{block}{}
		\justifying
		\small{Considere testar \(H_{0}: \theta \leq \theta_0\) versus \(H_{1}: \theta > \theta_0.\)
			Seja \(L(\theta; T(\vec{X}))\) a função de verossimilhança expressa em termos de uma
			estatística suficiente \(T = t(\vec{X})\) para \(\theta\), tal que, para quaisquer \(\theta_0 < \theta_1\) em \(\Theta,\)
			a razão de verossimilhança
			\[
			\Lambda(T(\vec{X}); \theta_0, \theta_1) =
			\frac{L(\theta_0; T(\vec{X}))}{L(\theta_1; T(\vec{X}))}
			\]
			é uma função monótona (não decrescente ou não crescente) em \(T(\vec{X})\),
			isto é, a família possui a propriedade de Razão de Verossimilhança Monótona (RVM). Sob tais condições, para um valor apropriado \(t_{0},\)
			\[
			C = \{\vec{X} : T(\vec{X}) > t_{0}\}
			\]
			é uma região crítica para um teste \textbf{uniformemente mais poderoso (UMP)}
			de nível \(\alpha = P_{\theta_{0}}(T(\vec{X}) > t_{0})\),
			para \(H_{0}: \theta \leq \theta_0\) versus \(H_{1}: \theta > \theta_0.\)}
	\end{block}
\end{frame}
...
\begin{block}{}
\justifying
Sabemos que $T(\Vec{X})=\Sumi X_{i}$ é uma estatística suficiente para $\theta$ e que a distribuição de $T(\Vec{X})=\Sumi X_{i}$ é Gamma$(n,\theta).$ Para $0<\theta_{1}<\theta_{0}<\infty$
\begin{align*}
    \dfrac{L(t;\theta_{0})}{L(t;\theta_{1})}=\left(\dfrac{\theta_{1}}{\theta_{0}}\right)^{n}e^{t\left(\frac{1}{\theta_{1}}-\frac{1}{\theta_{0}}\right)},~\frac{1}{\theta_{1}}-\frac{1}{\theta_{0}}>0, 
\end{align*}
...
\begin{block}{}
\justifying
Pelo teorema de Karlin-Rubin, o teste que rejeita $H_{0}$ para toda amostra $\Vec{X}$ tal que $\Sumi X_{i}<t_{0}$ é um teste uniformemente mais poderoso de tamanho $\alpha=P_{\theta_{0}}(\Sumi X_{i}<t_{0}).$
\end{block}
\pause
\begin{block}{}
\justifying

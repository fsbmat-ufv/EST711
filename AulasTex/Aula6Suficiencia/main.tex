...
%\begin{block}{Função Perda}
%		\justifying 
%		Quando $\delta(y)$ é um estimador de $\theta$ associamos a cada par $(\theta, \delta(y))$, $\theta \in \Omega$, um número não negativo $L(\theta, \delta(y))$ que reflete o quanto $\delta(y)$ está afastado de $\theta.$ Quando $\delta(y)$ é uma \textbf{ação} (ex.: 0 ou 1), 
%		$L(\theta, \delta(y))$ mede o custo de uma decisão incorreta.
%		\begin{itemize} 
...
%
%\begin{frame}{Função Perda: dois contextos}
%	\begin{block}{Definição 1 — Estimação}
%		Quando $\delta(Y)$ é um estimador de $\theta$, 
%		a função perda mede o \textbf{afastamento entre o valor verdadeiro e a estimativa}.
%		\[
...
%\end{frame}
%
%\begin{frame}{Função Risco}
%	
%	\begin{block}{Função Risco}
%		\justifying
%		Para a variável aleatória contínua $Y$, podemos calcular o risco:
%		\[
%		R(\theta, \delta) = \mathbb{E}\!\left(L(\theta, \delta(Y))\right)
%		= \int_{-\infty}^{\infty} L(\theta, \delta(y)) \cdot f_Y(y)\, dy.
%		\]
%	\end{block}
%\end{frame}
...
%		\justifying
%		Associamos a cada par $(\theta, \delta(y))$, $\theta \in \Omega$, um número não negativo 
%		$L(\theta, \delta(y))$ que representa o \textbf{custo da decisão} $\delta(y)$ 
%		quando o estado verdadeiro é $\theta$. 
%		\begin{itemize}
%			\item Chamamos a função $L$ de \textbf{função perda}.
%		\end{itemize}
%	\end{block}
%	\pause
